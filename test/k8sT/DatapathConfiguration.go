...
func testHostFirewall(kubectl *helpers.Kubectl) {
	...
	defer func() {
		By(fmt.Sprintf("Removing policies %s", demoHostPolicies))
		_, err = kubectl.CiliumPolicyAction(randomNs, demoHostPolicies, helpers.KubectlDelete, helpers.HelperTimeout)
		ExpectWithOffset(1, err).Should(BeNil(), fmt.Sprintf("Error removing resource %s: %s", demoHostPolicies, err))
	}()
